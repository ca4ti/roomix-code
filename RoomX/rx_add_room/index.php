...
function _moduleContent(&$smarty, $module_name)
{
    ...
    $pDBM = "mysql://root:eLaStIx.2oo7@localhost/roomix";
...
function saveAddRoom($smarty, $module_name, $local_templates_dir, &$pDB, &$pDBM, $arrConf, $arrLang)
{
	...
	$ch_selected=$_DATA['room'];

       foreach($ch_selected as $key => $value)
	{
       	$room_idx = $pAddRoom -> getAddRoom('','','extension',$value);
       	$room_name = $room_idx[0];
       	$arrValores['extension'] = "'".$value."'";
       	$arrValores['room_name'] = "'".$room_name['name']."'";
       	$arrValores['model'] = "'".$_DATA[intval($value)]."'";

              $arrRoomExist = $pAddRoomModel->getModel('rooms','','','extension',$value);
		$RoomExist=0;
              if (is_array($arrRoomExist))
			$RoomExist=1;

		if ( $RoomExist == 1 ) {
       		$save_rooms = $pAddRoomModel->updateQuery('rooms',$arrValores);
			}
		else	{
       		$save_rooms = $pAddRoomModel->insertQuery('rooms',$arrValores);
		}
...
function reportAddRoom($smarty, $module_name, $local_templates_dir, &$pDB, &$pDBM, $arrConf, $arrLang)
{
    ...
    if(is_array($arrResult) && $total>0){
        foreach($arrResult as $key => $value){
           ...
           $arrRooms = $pAddRoomModel->getModel('rooms','', '', $value['extension'], $filter_value);
           $chk=0;
	    ...
	    $select="";
    	    if(is_array($arrModel) && $totalModel>0){
           	...
           	$arrRoom = $pAddRoomModel->getModel('rooms','', '', 'extension', $value['extension']);
              if (is_array($arrRoom)){
           		$Room = $arrRoom['0'];
